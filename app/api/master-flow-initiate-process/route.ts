...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Check if the campaign objective is 'auto' and explicitly set it to null
    // This ensures we send a literal null to the backend API instead of an empty string
    let campaignObjective = post_assessment_campaign_objective;
    if (post_assessment_campaign_objective === 'auto') {
      console.log('🎯 Setting campaign objective to literal NULL as "auto" was specified');
      campaignObjective = null;
    }

    // Add explicit debug logging for campaign objective value
    console.log('🎯 Campaign objective value:', {
      'Original value': post_assessment_campaign_objective,
      'Processed value': campaignObjective,
      'Type': typeof campaignObjective
    });
