...
function Onboarding({
  ...
  const [goal, setGoal] = useState<string>(userDetails?.goal || "customers")

  const [dbChangeRequested, setDbChangeRequested] = useState(false)
  const [showSuccessMessage, setShowSuccessMessage] = useState(false)

  // Function to analyze the website link
  ...
  const handleSave = async () => {
    if (userDetails) {
      ...
      const emptyKeys = Object.keys(details).filter(key => details[key as keyof Details]?.trim() === "")
      if (emptyKeys.length > 0) {
        ...
        emptyKeys.forEach(key => {
          errors[key as keyof InputErrors] = "This field is required"
        })
      ...
      if (resp.success) {
        // Analyze the website after successfully updating the details
        if (websiteLink) {
          await analyzeWebsite(websiteLink)
        }
