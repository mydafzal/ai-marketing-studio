      ...
      <CardContent className="p-6">
        ...
        {targeting.facebook_positions?.length > 0 && (
          <div className="flex items-start gap-3 mb-3">
            <Facebook className="size-5 text-blue-500 shrink-0 mt-1" />
            <div>
              <span className="font-medium text-zinc-300">Facebook Placements:</span>{' '}
              <span className="text-zinc-400">{targeting.facebook_positions?.join(', ')}</span>
            </div>
          </div>
        )}
        ...
        {targeting.instagram_positions?.length > 0 && (
          <div className="flex items-start gap-3">
            <Instagram className="size-5 text-pink-500 shrink-0 mt-1" />
            <div>
              <span className="font-medium text-zinc-300">Instagram Placements:</span>{' '}
              <span className="text-zinc-400">{targeting.instagram_positions?.join(', ')}</span>
            </div>
          </div>
        )}
      </CardContent>
...
export function PlacementTargeting({
  ...
  const renderSwitch = (target: TargetPositionOption) => (
    ...
    >
      ...
      />
      <span className="text-zinc-300 pl-3 font-medium">{target.label}</span>
    </div>
  )
        ...
        <div className="space-y-4">
          <Card className="bg-zinc-900 border-zinc-800">
            <CardHeader>
              ...
              <div className="flex items-center gap-2 mt-2 text-sm text-blue-200 bg-blue-900/20 p-3 rounded-lg border border-blue-800">
                ...
                <Card className="bg-zinc-900 border-zinc-800">
                  <CardHeader className="pb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="font-semibold text-zinc-200">Facebook Placements</h3>
                    </div>
                  </CardHeader>
                ...
                <Card className="bg-zinc-900 border-zinc-800">
                  <CardHeader className="pb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="font-semibold text-zinc-200">Instagram Placements</h3>
                    </div>
                  </CardHeader>
                  <CardContent className="space-y-1">
                    {targetPositions
